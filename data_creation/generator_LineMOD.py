...
total_set = 10 #10000 set of scenes, each set has identical objects with varied poses to anchor pose (+-15)
pair_set = 1 #number of pair scene for each set, 10
sample_dir = '/home/stefan/data/rendered_data/linemod_debug' #directory for temporary files (cam_L, cam_R, masks..~)
target_dir = '/home/stefan/data/rendered_data/linemod_debug/patches'
index=0
isfile=True
while isfile:
...
for num_set in np.arange(total_set):
    ...
    for i in np.arange(Back_object):
        ...
        if draw < 0:
            anchor_pose[i+num_object,0] = - 0.35 + draw
        else:
            anchor_pose[i+num_object,0] = 0.35 + draw 
        draw = uniform(-1, 1) * 0.2
        if draw < 0:
            anchor_pose[i+num_object,1] = draw
        else:
            anchor_pose[i+num_object,1] = 0.25 + draw
        anchor_pose[i+num_object,2] =0.3 + 0.2*float(i)
        #anchor_pose[i,2] = 0.1 + random()*0.2
    ...
    for iii in np.arange(pair_set):
        ...
        scene.frame_set(0)
        for obj in scene.objects:
            print(obj_object.name, obj_object.pass_index)
            if obj.type == 'MESH':
            ...
            if obj.name == 'InvisibleCube':
                obj_object.rotation_euler.x=radians(random()*65.0+10.0) #0~90
            ...
            if obj.type == 'CAMERA' and  obj.name=='cam_L':
                obj_object = bpy.data.objects[obj.name]
                obj_object.location.z = 0.65 + random()+0.5  #1.0-2.5
                ...
                for ob in scene.objects:
                    if ob.type == 'CAMERA':
                        if ob.name=='cam_L': #ob.name =='mask':
                            ...
                            print('Render individual mask: ', ob.name )
