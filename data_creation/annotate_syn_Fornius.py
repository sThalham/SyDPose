...
if __name__ == "__main__":
    ...
    root = '/home/stefan/data/rendered_data/fronius20/patches/'
    #root2 = '/home/sthalham/data/renderings/fronius_22_2/patches/'
    target = '/home/stefan/data/train_data/fronius20/'
    ...
    for fileInd in syns:
        if fileInd.endswith(".yaml"):
            ...
            for k in range(0, 1):
                ...
                fileName = target + "images/train/" + newredname + '.png'
                ...
                if myFile.exists():
                    ...
                    if method == 'normals':
                        ...
                        #                    augmentation_var)
                        #aug_xyz, depth_refine_aug, depth_imp = get_normal(depthAug, fx=fxkin, fy=fykin, cx=cxkin,
                        #                                                  cy=cykin, for_vis=False)

                        #depthAug[depthAug > depthCut] = 0
                        #scaCro = 255.0 / np.nanmax(depth_refine)
                        #cross = np.multiply(depth_refine, scaCro)
                        #depth_mask = cross.astype(np.uint8)
                        #cv2.imwrite(fileName, depth_mask)

                        depth_mask = np.repeat(depth_refine[:,:, np.newaxis], 3, axis=2)
                        depth_mask[:, :, 2] = mask
